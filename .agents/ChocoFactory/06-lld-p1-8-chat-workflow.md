...
1. Ship `workflows/chat.yaml` (§5.4) — checked into the repo, embedded
   into the `chocofactoryd` binary, and seeded (write-if-missing) into
   the user's own `~/.config/chocofactory/workflows/` on first run (§2.6
...
Everything here is new library-level surface (`chocofactoryd` crate, no
...
### 2.2 `chocofactoryd/src/global_config.rs` (new module)
...
impl GlobalConfig {
    ...
    /// `$HOME/.config/chocofactory/config.yaml`, or `None` if `$HOME`
...
### 2.3 `chocofactoryd/src/role_config.rs` (new module)
...
### 2.4 Modifies `chocofactoryd/src/workflow_def.rs`
...
to a small shared helper — e.g. `chocofactoryd/src/fileref.rs::resolve_relative`
...
### 2.6 Config root, defaults, and seeding built-in workflows (new module, `chocofactoryd/src/config_root.rs`)
...
single place that decides "where does chocofactory's home live," not two
copies of the `$HOME/.config/chocofactory` prefix that could drift apart.
...
`workflows_dir` here is always the seeded `~/.config/chocofactory/workflows/`
...
seed_builtin_workflows(workflows_dir)        (once, at daemon startup — §2.6)
  -> ~/.config/chocofactory/workflows/chat.yaml written, if not already present

create_task("chat", "hey, look into the flaky test", config: json!({}))
  -> resolve_workflow_path("chat")           -> ~/.config/chocofactory/workflows/chat.yaml
...
once elsewhere. A hand-edited `~/.config/chocofactory/config.yaml` with a
